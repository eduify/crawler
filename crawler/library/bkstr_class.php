...
class bkstr {
    ...
    private function MainBookData($url, $initial_csv_row_data, &$output) {
        include_once("simple_html_dom.php");
        $Main_Data = "";
            ...
            for ($j = 0; $j < $total_type_books; $j++) {
                ...
                for ($i = 0; $i < $total_books; $i++) {
                    ...
                    if ($SisterUrl_Ancher->plaintext != "") {                                   // Check if Sister URL is available
                        ...
                        $sister_site_data = $this->SisterSiteData($SisterUrl);
                    } else {
                        $sister_site_data = ",,,,,,";
                    }
                    if ($ImageUrl <> "http://images.efollett.com/books/noBookImage.gif") {  // ONly Access Amazon Api if you image FOund
                        ...
                        $Bk_ISBN = $Bk_ISBN[0];
                        $amazon = $this->getAmazonData("$Bk_ISBN", "ItemLookup");
                        if ($amazon) {
                            ...
                            $amazon = $this->getAmazonData("$BookTitle, $Author, $Edition", "ItemSearch");
                            if ($amazon) {
                                $AmazonListPrice = $amazon['AmazonListPrice'];
                                $AmazonDiscountPrice = $amazon['AmazonDiscountPrice'];
                                $NonAmazonNewPrice = $amazon['NonAmazonNewPrice'];
...
//--------------------------------------------------------------------------------------------------------
    private function SisterSiteData($sister_url) {
        include_once("simple_html_dom.php");
        $url = $sister_url;
        $html = file_get_dom($url);
...
//------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------
    public function getStateUniversity($state) {
...
//------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------
    public function getCampusUniversity($University) {
    ...
    public function getBK_BOOKS_URL($campus) {
...
//------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------
    public function runScrapper($Store, $University_Name, $Campus_Name) {
        if (PHP_OS == "WINNT") {
                        ...
                        foreach ($Department_arr as $Department_Name => $Department_Value) {
                            ...
                            foreach ($Course_arr as $Course_Name => $Course_Value) {
                                ...
                                foreach ($Section_arr as $Section_Name => $Section_Value) {
                                    ...
                                    $this->MainBookData($FinalUrl, $initial_csv_row_data, $output);
...
while ($condition) {
    switch ($option) {
        case "":
            $option = $bkstr->getOptions();           // General Options
            break;
        case 1:
            ...
            $state = $bkstr->getState();
            $University = $bkstr->getStateUniversity($state);
            $Campus = $bkstr->getCampusUniversity($University[0]);
            $Store = $bkstr->getBK_BOOKS_URL($Campus[0]);

            $option = $bkstr->getOptions();
            break;
        case 2:
            if ($Store <> "") {
                ...
                $bkstr->runScrapper($Store, $University[1], $Campus[1]);
            }
            $option = $bkstr->getOptions();   // General Options
            break;
        case 3:
